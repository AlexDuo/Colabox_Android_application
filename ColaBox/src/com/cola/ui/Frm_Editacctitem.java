...
public class Frm_Editacctitem extends ExpandableListActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setTitle("Duo Billing-ChooseBill");
	...
	public boolean onContextItemSelected(MenuItem item) {
		...
		if (item.getItemId() == 1) {
			...
			newdialog = new Dialog_edit(this, "Please input the Bill's name", "",
					mDialogClick_new);
			newdialog.show();
		} else if (item.getItemId() == 2) {
			new AlertDialog.Builder(this).setTitle("Notice").setMessage("Sure to delete'"+((TextView)info.targetView).getText().toString()+"'This Bill?")
					.setIcon(R.drawable.quit).setPositiveButton("Sure",
							...
							new DialogInterface.OnClickListener() {
								public void onClick(DialogInterface dialog,
										int whichButton) {
									// Cancel the button event
								}
							}).show();

		} else if (item.getItemId() == 3) {
			newdialog = new Dialog_edit(this, "Please update the bill's name",
